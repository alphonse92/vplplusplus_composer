...
public class CasaDeCambio {
    ...
    public float getGananciaEnUnBolivar() {
        return getPrecioDeVenta()-getPrecioDeCompra();//COMPLETE
    }
    ...
    public boolean cambiarPrecioDelBolivar(float precioDeCompra, float precioDeVenta) {
        boolean precioValido = precioDeCompra > 0 && precioDeVenta > 0 && precioDeVenta>precioDeCompra;//COMPLETE
        if(precioValido){
    ...
    public boolean comprarBolivares(int cantidad) {
        boolean puedeComprar = getPrecioDeCompra()*cantidad <= getPesosEnCaja();//COMPLETE
        if(puedeComprar){
            this.bolivaresEnCaja += cantidad;
            this.pesosEnCaja -= (getPrecioDeCompra()*cantidad);
    ...
    public boolean venderBolivares(int cantidad) {
        boolean puedeVender = cantidad <= getBolivaresEnCaja();//COMPLETE
        if(puedeVender){
            this.bolivaresEnCaja -= cantidad;
            this.pesosEnCaja += (getPrecioDeVenta()*cantidad);
    ...
    public float getImpuestos() {
        return ((getBolivaresVendidos()*getPrecioDeVenta())*16)/100;//COMPLETE
    }//fin getImpuestos
    ...
    public float getGanancias() {
        return getPesosEnCaja()-getImpuestos();//COMPLETE
    }//fin getGanancias
    ...
    public void inyectarPesos(int cantidad) {
        if(cantidad > 0)this.pesosEnCaja += cantidad;//COMPLETE
    }//fin inyectarPesos
    ...
    public void inyectarBolivares(int cantidad) {
        if(cantidad > 0)this.bolivaresEnCaja += cantidad;//COMPLETE
    }
