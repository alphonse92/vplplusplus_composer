...
gananciasNetas = 0;
    ...
    public int calcularPrecio(int tipoCafe, int cantidadAzucar) {
        int precioParcial = precioBaseCafe / 1000 * 55 * tipoCafe;
    ...
    public boolean prepararCafe(int tipoCafe, int cantidadAzucar) {
        ...
        if (cafe >= tipoCafe*55 && azucar >= 5*(azucar-1) && vasos > 0){
            siHayInsumos = true;
            cafe -= tipoCafe*55;
            azucar -= 5*(azucar-1);
            --vasos;
