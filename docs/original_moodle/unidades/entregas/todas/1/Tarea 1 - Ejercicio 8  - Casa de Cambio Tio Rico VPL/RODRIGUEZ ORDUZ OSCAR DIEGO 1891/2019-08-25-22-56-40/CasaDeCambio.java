...
public class CasaDeCambio {
    ...
    public float getGananciaEnUnBolivar() {
        float gananciaEnBolivar = getPrecioDeVenta() - getPrecioDeCompra();
        return gananciaEnBolivar;//COMPLETE
    }
    ...
    public boolean cambiarPrecioDelBolivar(float precioDeCompra, float precioDeVenta) {
        ...
        this.precioDeVenta = precioDeVenta;//COMPLETE
        if((getPrecioDeCompra() > 0 &&  getPrecioDeVenta() > 0) && (getPrecioDeCompra() < getPrecioDeVenta())){
            precioValido = true;
        }
        return precioValido;
    ...
    public boolean comprarBolivares(int cantidad) {
        ...
        if (cantidad > 0 && (cantidad * precioDeCompra) <= pesosEnCaja){
    ...
    public float getImpuestos() {
        float impuestos = (bolivaresVendidos * precioDeVenta) * 0.16f;
