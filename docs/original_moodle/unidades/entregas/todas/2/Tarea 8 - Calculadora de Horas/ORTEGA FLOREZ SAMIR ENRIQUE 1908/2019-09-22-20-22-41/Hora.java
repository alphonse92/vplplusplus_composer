...
public class Hora
{
    ...
    protected boolean esHoraValida(){
        ...
        if(HH24.equals(formato)){
            if((horas>=1&&minutos>=0&&segundos>=0)&&(horas<=23&&minutos<=59&&segundos<=59)&&ampm==null){
                valido =true;
            }
        }else if(HH12.equals(formato)){

            if((horas>=1&&minutos>=0&&segundos>=0)&&(horas<=12&&minutos<=59&&segundos<=59)){
                valido=true;
            } 
    ...
    public Hora getHoraHH24(){
        Hora h12 = new Hora(this.horas , this.minutos , this.segundos , this.formato ,this.ampm) ; 
    ...
    public Hora getHoraHH12(){
        Hora h24 = new Hora(this.horas , this.minutos , this.segundos , this.formato , this.ampm ) ; 
        if(this.formato.equals(HH24)){
            h24.setFormato(HH12); 
        }
        return h24;
    }
    ...
    public int restar(Hora other){
        int resta = 0 ; 
        Hora t1 = this.getHoraHH24(); 
        Hora t2 = other.getHoraHH24(); 
        resta = (t1.horas *3600 + t1.minutos * 60 + t1.segundos ) - (t2.horas *3600 + t2.minutos * 60 + t2.segundos ) ; 
    ...
    public boolean esIgual(Hora other){
        ...
        if( this.restar(other) == 0 )esIgual = true ; 
        return esIgual ;//COMPLETE
    }
    ...
    public boolean esMenor(Hora other){
        ...
        if( this.restar(other) < 0 )esMenor = true ; 
        return esMenor ;//COMPLETE
    }  
    ...
    public boolean esMayor(Hora other){
        ...
        boolean esMayor = this.restar(other) > 0 ; 
    ...
    public String toString(){
        ...
        if( this.formato.equals(HH24))
            ...
            str = this.horas +" : "+this.minutos+" : " + this.segundos+" "+this.ampm;
        return str;//COMPLETE
    }
    ...
    public void setFormato(String formato){
        if(!formato.equals(this.formato)){ // si el formato es diferente a this.formato haga algo , de lo contrario no haga nada 
            // pasar de formato HH24 a HH12
            if(formato.equals(HH12)){
                ...
                // pasar de formato HH12 a hh24
            }else if(formato.equals(HH24)){
                this.formato=HH24;
                if(ampm.equals(AM)&&horas==12){
                    horas-=12;
                }else if((ampm.equals(AM)&&horas>=1)||(ampm.equals(PM)&&horas==12)){
                    horas=horas;  
                }else if(ampm.equals(PM)&&horas>=1&&horas<=11){
                    horas+=12;
                }
                ampm=null;
            }   
