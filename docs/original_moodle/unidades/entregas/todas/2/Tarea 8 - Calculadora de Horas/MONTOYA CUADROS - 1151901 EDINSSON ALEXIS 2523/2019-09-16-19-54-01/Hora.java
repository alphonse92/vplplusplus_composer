...
public class Hora
{
    ...
    protected boolean esHoraValida(){
        ...
        if(formato=="HH12"){
            if(ampm=="am" && horas>0 && horas <= 12) validador = true;
            if(ampm=="pm" && horas<=12 && horas<0) validador = true;
            if(minutos>=0 && minutos < 60) validador = true;
            if(segundos>=0 && segundos < 60) validador = true;
        }
        if(formato=="HH24"){
            if(horas>=0 && horas < 24) validador = true;
            if(minutos>=0 && minutos < 60) validador = true;
            if(segundos>=0 && segundos < 60) validador = true;
        }
        return validador;//COMPLETE
    }
    ...
    public Hora getHoraHH24(){
        Hora hora1 = new Hora();
        hora1.minutos = this.minutos;
        hora1.segundos = this.segundos;
        
        if(formato != "HH24"){
            if(horas >= 0 && horas < 12 && ampm == "am") hora1.formato = "HH24";
            if(horas > 0 && horas < 12 && ampm == "pm"){
                hora1.horas = 12 + hora1.horas;
                hora1.formato = "HH24";
            }
            if(horas == 12 && ampm == "pm") hora1.formato = "HH24";
        }
        return hora1;//COMPLETE
    }
    ...
    public int restar(Hora other){
        // Las variables 'a' y 'b' se usan para almacenar los valores de las horas convertidos en segundos.
        other = new Hora();
        int a = (horas*3600)+(minutos*60)+(segundos);
        int b = (other.horas*3600)+(other.minutos*60)+(other.segundos);
        return a-b;//COMPLETE
    }
